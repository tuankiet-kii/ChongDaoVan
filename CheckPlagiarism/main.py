...
#hàm so sánh sử dụng thư viên difflib
def usedifflib(str1,str2):
    rs = str(int(difflib.SequenceMatcher(None,str1,str2).ratio()*100)) + "%"
    return rs
...
def timkiem():
    ...
    driver = webdriver.Chrome(chrome_options=chrome_options, executable_path="chromedriver1.exe")
    dem = 0
    d = 0
    tk = convert(box1.get(1.0, END)).split()
    for tkk in tk:
        d += 1
    count = 0
    for i in search(box1.get(1.0,END),stop=1,lang="vn"):
        dem+=1
        if dem == 1:
            btn1.insert(END,i)
            count += 1
            driver.get(i)
            noidungs = driver.find_elements_by_tag_name('p')
            ketqua = 0
            for ct in noidungs:
                demm = 0
                for t in tk:
                    for ch in str(ct.text).split():
                        if (t.__contains__(ch)):
                            demm += 1

                if (demm > d * 2):
                    ketqua = 1
                    txt1.insert(END,kiemtradaovan1(ct.text))
                    continue
        if dem == 2:
            btn2.insert(END,i)
            count += 1
            ...
            for ct in noidungs:
                ...
                if (demm > d * 2):
                    ketqua = 1
                    txt2.insert(END, kiemtradaovan1(ct.text))
                    continue
        if dem == 3:
            btn3.insert(END, i)
            count += 1
            driver.get(i)
            noidungs = driver.find_elements('p')
            ketqua = 0
            for ct in noidungs:
                demm = 0
                for t in tk:
                ...
                if (demm > d*1.5):
                    ketqua = 1
                    txt3.insert(END, kiemtradaovan1(ct.text))
                    continue
        if dem == 4:
            btn4.insert(END,i)
            count += 1
            ...
            for ct in noidungs:
                ...
                if (demm > d * 2):
                    ketqua = 1
                    txt4.insert(END, kiemtradaovan1(ct.text))
                    continue
        if dem == 5:
            btn5.insert(END,i)
            count += 1
            ...
            for ct in noidungs:
                ...
                if (demm > d * 2):
                    ketqua = 1
                    txt5.insert(END, kiemtradaovan1(ct.text))
                    break
...
#hàm so sánh sử dụng thư viện nltk
...
Button(app,text="Tìm kiếm",font=("Arial",12),bg="green",fg="white",command=timkiem).place(x=500,y=330)

Button(app,text="Xóa",font=("Arial",12),bg="red",fg="white",width=5,command=clear).place(x=650,y=330)
...
Button(app,text="Tra cứu",font=("Arial",10),bg="green",fg="white",command=tracuu1).place(x=1100,y=380)
...
Button(app,text="Tra cứu",font=("Arial",10),bg="green",fg="white",command=tracuu2).place(x=1100,y=450)
...
Button(app,text="Tra cứu",font=("Arial",10),bg="green",fg="white",command=tracuu3).place(x=1100,y=520)
...
Button(app,text="Tra cứu",font=("Arial",10),bg="green",fg="white",command=tracuu4).place(x=1100,y=590)
...
Button(app,text="Tra cứu",font=("Arial",10),bg="green",fg="white",command=tracuu5).place(x=1100,y=660)
